...
void *handle_connection(void *arg){
	...
	else
	{
		...
		//Send from cache
		if(index != -1){
			send(client_socket, cache[index].content, strlen(cache[index].content), 0);
			close(client_socket);
			pthread_mutex_unlock(&mutex);
			pthread_exit(NULL);
		}
		...
		//If cache is full, add to the next available spot
		if(cache_size < 5){
			cache[cache_size].filename = filename;
			cache[cache_size].content = content;
			cache_size++;
		}
		//Otherwise remove the oldest element and add a new one to the end.
		//The other elements are shifted to presere the order they arrived.
		else{
			free(cache[0].content);
			for(int i = 0; i < 4; i++){
				cache[i].filename = cache[i+1].filename;
				cache[i].content = cache[i+1].filename;
			}
			cache[4].filename = filename;
			cache[4].content = content;
		}
