...
int main()
{
	...
	for(;;)
	{
		...
		//Create a new process using fork() to handle the request and response
		int pid = fork();
		if(pid < 0){
			perror("Fork failed");
			close(connfd);
		...
		//Close connection file descriptor in parent process
		if(pid > 0)
			close(connfd);
		...
		//Close socket file descriptor in child process
		close(sfd);
